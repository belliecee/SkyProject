...
class VendorprocessController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('patchinsert','create','update','admin','delete','autocomplete'),
				'users'=>array('@'),
			),
		...
		public function actionPatchinsert(){
			$model = vendorprocess::model()->findAll();
			foreach($model as $mo){
				$projectmodel = project::model()->findByPk($mo->project_id);
				if($projectmodel != null){
					$projectmodel->vendor_id = $mo->vendor_id;
				}
				$projectmodel->save();
			}
	...
	{
		$model=new vendorprocess;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['vendorprocess']))
		{
			$model->attributes=$_POST['vendorprocess'];
	...
	public function actionIndex($project_id)
	{
		$criteria = new CDbCriteria();
				...
				$model = vendorprocess::model()->find($criteria);
                
                if($model === null){
                     $newmodel = new vendorprocess;
	...
	{
		$model=new vendorprocess('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['vendorprocess']))
			$model->attributes=$_GET['vendorprocess'];
	...
	{
		$model=vendorprocess::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='vendorprocess-form')
		{
