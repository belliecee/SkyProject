...
class QuohController extends Controller
{
        ...
        // Query with 
         ...
         // Append the query result to array
          ...
          if($fieldname == "status")
          {
              ...
              $somequoh = quoh::model()->findAll("project_id=:project_id AND status=:status",array(":project_id"=>$proj->id,":status"=>2));
              if($somequoh != null){
                  $proj->status = 19;
              }else{
                   $proj->status = 4;
  ...
  */        Yii::app()->end();
        ...
        public function actionDeleteQuoh($id,$project_id)
	{
                ...
                if($model != null){
                    $thisstatus = $model->status;
                    quod::model()->deleteAll("quoH_id=:id",array(":id"=>$id));
                    $model->delete();
                    if($thisstatus == 2){
                            $proj = project::model()->findByPk($project_id);
                            $somequoh = quoh::model()->findAll("project_id=:project_id AND status=:status",array(":project_id"=>$project_id,":status"=>2));
                            if($somequoh != null){
                                $proj->status = 19;
                            }else{
                                 $proj->status = 4;
